...
**Fork exception:** the `docs/` tree is upstream's published content (aihero.dev), so fork-only skills (`architecture-diagram`, `zoom-out`, `caveman`, `structresponse`, `coding-standards`, `fix-from-evidence`, `security-audit`) get **no** docs page; only touch `docs/` via upstream merges. Everything else above (README/plugin.json wiring, `agents/openai.yaml`, invocation split, `ask-matt` routing) applies to fork-only skills too.
...
To (re)link every skill outside `deprecated/` and `in-progress/` into the local harness skill directories (`~/.claude/skills`, `~/.agents/skills`), run `scripts/link-skills.sh`. Each entry is a symlink into this repo, so a `git pull` keeps installed skills current; re-run the script after adding, removing, or renaming a skill.
