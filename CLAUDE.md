...
Each bucket folder has a `README.md` that lists every skill in the bucket with a one-line description, with the skill name linked to its `SKILL.md`. The promoted buckets' `README.md`s group entries into **User-invoked** and **Model-invoked**; the top-level `README.md` and non-promoted bucket `README.md`s (`misc/`, `personal/`) use a flat list.
